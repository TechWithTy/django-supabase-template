...
def run_verification():
    ...
    logger.info(
        f"Client import verification: {'✓ Passed' if import_ok else '✗ Failed'}"
    )
    ...
    logger.info(
        f"Client initialization verification: {'✓ Passed' if init_ok else '✗ Failed'}"
    )

    if not init_ok:
        logger.error(
            "Cannot continue verification without successful client initialization"
        )
        return False
    ...
    logger.info(
        f"Overall result: {'✓ All verifications passed' if all_passed else '✗ Some verifications failed'}"
    )
