...
"""
    ...
    def test_script_execution_failure(self, authenticated_client, create_test_script, user_with_credits):
        ...
        # Make the request with a parameter that will cause the script to fail
        ...
        # Check response
        assert response.status_code == status.HTTP_200_OK  # Still returns 200 even on script failure
        assert response.data['success'] is False
        assert response.data['exit_code'] == 1
        assert 'Error: Script failed with an exception' in response.data['stderr']
        
        # Verify the user's credits were NOT deducted (script failed)
        profile = UserProfile.objects.get(user=user_with_credits)
        assert profile.credits_balance == 10  # Still has all credits
        
        # Verify no transaction was recorded
        assert CreditTransaction.objects.filter(user=user_with_credits).count() == 0
    ...
    def test_invalid_credit_amount_parameter(self, authenticated_client, create_test_script, user_with_credits):
        ...
        # Check response - should succeed because parameter is ignored
        assert response.status_code == status.HTTP_200_OK
        assert response.data['success'] is True
        assert response.data['credits_used'] == 5  # Default credit cost is used
    ...
    def test_exact_credit_balance(self, exact_credit_client, create_test_script, user_with_exact_credits):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data['success'] is True
        assert response.data['credits_used'] == 5
        assert response.data['credits_remaining'] == 0  # Used all credits
        
        # Verify the user's credits were deducted to exactly 0
        profile = UserProfile.objects.get(user=user_with_exact_credits)
        assert profile.credits_balance == 0
